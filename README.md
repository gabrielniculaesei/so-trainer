...
Web app statica per prepararsi allo scritto e all’orale di Sistemi Operativi, con quiz, esercizi, mappa mentale della teoria, simulazioni interattive degli algoritmi, simulazioni d’esame e flashcard di teoria con ripetizione dilazionata.
...
| `data/generators.js`, `data/generators2.js`, `data/generators3.js` | 31 generatori di esercizi con parametri casuali e soluzione calcolata: FAT, i-node, EAT/TLB, scheduling disco e CPU, sostituzione pagine, RAID/XOR, pipeline, semafori, aging, legge di Amdahl, overhead delle interruzioni, costo delle system call… |
| `data/data_mappa.js`, `js/mappa.js` | Mappa mentale del corso: 127 concetti in un albero navigabile (radice → 4 capitoli → sezioni → concetti), ognuno con spiegazione, nota “all'esame” e collegamenti a quiz, flashcard e simulazione; 29 nodi hanno anche il codice (Peterson, TSL/XCHG, semafori, monitor, filosofi, lettori–scrittori, Clock, aging, EAT, i-node, XOR…) |
...
* **Mappa**: la mappa mentale della teoria. Apri i rami, clicca un concetto e leggi la spiegazione nel pannello, con la nota su cosa viene chiesto all'esame e i collegamenti al quiz, alle flashcard e alla simulazione dell'argomento. Dove serve trovi il codice del corso (TSL, Peterson, produttore–consumatore con i semafori, monitor, filosofi a cena…) e i calcoli tipici d'esame. Cerca un concetto per nome, segna come “ripassato” ciò che sai già e usa la barra di avanzamento per vedere quanto manca: è il ripasso d'insieme da fare nei giorni prima dell'esame.
